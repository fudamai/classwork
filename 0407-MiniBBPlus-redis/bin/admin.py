...
client = redis.Redis()
channels = ["show", "add", "delete"]  # 创建redis的客户端
...
class Admin():
    ...
    def query_article_to_json(self, id, mode=1):
        ...
        if mode == 1:
            ...
            for i in result:
                ...
                ar_all = json.dumps({'status':0, 'behavior':'show all article', 'statusText':'所有文章数据', 'articles':article_list}, ensure_ascii=False, separators=(',', ':'), indent=4)
            print('输出全部文章')
            return ar_all
    ...
    def auto_add(self, title, content, author_name):
        ...
        自动填入表单，并提交入数据库
        """
        author_exist_judge = self.session.query(Authors).filter(Authors.name == author_name)[0]
        if author_exist_judge:  # 判断author是否存在
            author = author_exist_judge
        else:
...
def main():
    ...
    if args.show:
        result = tool.show()
        print(result.all())
        client.publish("show", json.dumps({'status':0, 'behavior':'show', 'article':result.all()}, ensure_ascii=False))
    elif args.add:
        title = args.add[0]
        content = args.add[1]
        author_name = args.add[2]
        tool.auto_add(title, content, author_name)
        client.publish("add", json.dumps({'status':0, 'behavior':'add', 'article':{'title':title, 'content':content, 'author_name':author_name}}, ensure_ascii=False))
    elif args.delete:
        id = args.delete[0]
        result = tool.delete(id)
        client.publish("delete", json.dumps({'status':0, 'behavior':'delete', 'article':result}, ensure_ascii=False))

    # tool.query_article_to_json(2093061798)
